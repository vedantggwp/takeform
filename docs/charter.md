...
Every row is planned today. No command, test target or evidence artifact below exists. Each command shape and test target is proposed, and the PR that creates one may rename it and must update its row. Fixture ids M, T and L are the montage, speech and thirty-minute fixtures owned by [issue 19](https://github.com/vedantggwp/takeform/issues/19). S is a proposed fourth fixture for the story-first short, and issue 19 or a child of it decides whether S is new material or a variant of T. Status uses three words, planned, implemented and verified, defined in section 7. A later PR names the journey id it advances.
...
| J04 | Chaptered long film. From a chapter script, produce a thirty-minute chaptered episode with captions and audio across every chapter join, revise one chapter, then export it. | Script | Long-form | `takeform verify long --fixture L`, proposed | Exported film plus receipt, measurement JSON of seek latency, peak memory and export time | [#16](https://github.com/vedantggwp/takeform/issues/16), [#15](https://github.com/vedantggwp/takeform/issues/15), [#13](https://github.com/vedantggwp/takeform/issues/13) | planned |
...
| J14 | Fresh-machine setup. On a clean user account, clone the repository, run the doctor command, build and launch the development build, then separately install the signed bundle without the toolchain. | None, precedes entry | Both | `takeform doctor` on a clean account, then `swift build && swift test`, proposed | Review recording of the clean account, measurement JSON of the doctor output | [#5](https://github.com/vedantggwp/takeform/issues/5), [#18](https://github.com/vedantggwp/takeform/issues/18), [#16](https://github.com/vedantggwp/takeform/issues/16) | planned |
