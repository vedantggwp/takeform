...
assert hashlib.sha256(checkpoint.read_bytes()).hexdigest()==expected_hash, 'unapproved model bytes'
...
assert len(source)==receipt['finalizedFrames']==receipt['writtenFrames'], 'audio frame receipt mismatch'
...
for utterance in receipt['utterances']:
    ...
    assert np.max(np.abs(source[begin:end]))>0.003, 'silent utterance rejected before inference'
    ...
    assert [s.token for s in spans]==targets[0].tolist(),'CTC path coverage differs from normalized transcript'
    assert all(s.start<s.end for s in spans),'zero-duration CTC token'
    ...
    for part in parts:
        ...
        assert begin/rate<=a<b<=end/rate
