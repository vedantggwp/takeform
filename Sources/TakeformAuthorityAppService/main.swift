...
while true { let fd=accept(listen,nil,nil);guard fd>=0 else{continue}; Thread { defer{close(fd)}; guard authenticated(fd,requirement) else {try? AppAuthoritySocket.send(AppAuthorityResponse.failure(.creatorAuthorizationRequired),fd);return}; do { let request=try AppAuthoritySocket.receive(AppAuthorityRequest.self,fd); let response:AppAuthorityResponse
 ...
 case let .open(url, rebind, credential):
    ...
    let secret = String(decoding: credential, as: UTF8.self)
    let opened = try authority.openForAuthenticatedCreator(credential: secret, rebindMovedPackage: rebind)
    response = .snapshot(WorkspaceSnapshot(document: opened.document, projectionMatches: opened.projectionMatches, packageURL: url))
 case let .execute(url, envelope, credential):
    let authority = try ProjectAuthority(packageURL: url)
    let secret = String(decoding: credential, as: UTF8.self)
    response = .result(try authority.executeForAuthenticatedCreator(envelope, credential: secret))
 case let .pair(url, label, expires, credential):
    let authority = try ProjectAuthority(packageURL: url)
    let secret = String(decoding: credential, as: UTF8.self)
    let raw = token()
    let grant = try authority.issuePairedCLIGrant(credential: secret, label: label, scopes: [.editProject], expiresAt: expires, rawToken: raw)
    response = .pairing(grant.id, raw)
 case .revoke:
    throw WorkspaceFailure.rejected("Grant selection is not implemented")
 }
