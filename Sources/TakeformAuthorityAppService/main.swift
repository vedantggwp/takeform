...
while true { let fd=accept(listen,nil,nil);guard fd>=0 else{continue}; Thread { defer{close(fd)}; guard authenticated(fd,requirement) else {try? AppAuthoritySocket.send(AppAuthorityResponse.failure(.creatorAuthorizationRequired),fd);return}; do { let request=try AppAuthoritySocket.receive(AppAuthorityRequest.self,fd); let response:AppAuthorityResponse
 switch request {
 case let .open(url,rebind,credential): let authority=try ProjectAuthority(packageURL:url); let session=AuthorityAppServiceGate.session(creatorCredential:String(decoding:credential,as:UTF8.self)); try authority.establishCreator(session); let open=try authority.open(rebindMovedPackage:rebind); response = .snapshot(WorkspaceSnapshot(document:open.document,projectionMatches:open.projectionMatches,packageURL:url))
