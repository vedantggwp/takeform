...
/// Capability used only by the authenticated native-app service target.
/// It is deliberately SPI so the CLI has no ordinary issuer API.
@_spi(AuthorityAppService)
public struct AuthorityAppSession: Sendable {
    fileprivate init() {}
}

@_spi(AuthorityAppService)
public enum AuthorityAppPeer {
    /// The UDS service calls this only after it has verified the connected native app's code identity.
    public static func authenticatedSession() -> AuthorityAppSession { AuthorityAppSession() }
}

extension ProjectAuthority {
    @_spi(AuthorityAppService)
    public func issueCLIGrant(
        _ session: AuthorityAppSession,
        label: String,
        scopes: Set<GrantScope>,
        expiresAt: Date,
        rawToken: String
    ) throws -> Grant {
        let openState = try open()
    ...
    @_spi(AuthorityAppService)
    public func revokeCLIGrant(_ session: AuthorityAppSession, grantID: UUID) throws {
        let openState = try open()
        var state = try loadMachineState(for: openState.document.projectID)
        guard let index = state.grants.firstIndex(where: { $0.id == grantID }) else { return }
