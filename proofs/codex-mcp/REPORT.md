...
After independent review of the narrowed matcher, one separately authorized Terra acceptance attempt completed in about 14 seconds. It again produced two MCP elicitation requests and no Takeform tool events. Both requests failed the reviewed matcher and were declined. The historical receipt's `tokenUsage` object is last-request usage: 27,034 tokens, including 27,027 input, 25,344 cached input, and 7 output tokens. Total-turn usage was not captured and is unavailable. No retry ran.
...
After root reviewed that source-pinned repair, one authorized Terra acceptance turn passed in about 14 seconds. It issued exactly one model snapshot followed by the fixed proposal. Both approval requests satisfied every bounded predicate. The proposal remained pending at revision 1 until creator-channel acceptance, after which the MCP snapshot returned revision 2 with the replacement. Duplicate acceptance returned the same revision 2 result; stale and malformed proposals were rejected. The historical receipt's `tokenUsage` object is last-request usage: 26,945 tokens, including 26,938 input, 26,368 cached input, and 7 output tokens. Total-turn usage was not captured and is unavailable.
...
Future receipts distinguish `totalTurnUsage` from `lastRequestUsage`, captured from the same final `thread/tokenUsage/updated` notification. Historical receipts remain unchanged.
...
- The source-reviewed driver ran one authorized low-effort Terra turn. Exactly two approval requests passed all predicates; exactly two ordered model-origin tool events followed. Creator acceptance advanced the same live toy authority from revision 1 to revision 2, and the duplicate, stale, and malformed guards passed.

## Boundaries

The local tests prove the toy store mechanics and the client's request routing. The no-model receipt proves Codex app-server tool discovery and a live revision 1 read with explicit invocation configuration. The final real-turn receipt proves the positive approval policy and the model-to-proposal-to-creator-acceptance round trip against the live toy authority. It does not prove native Takeform authority, production persistence, creator UX, provider flexibility, or ChatGPT web connectivity.
