...
The installed app-server schema lists eleven server-request methods. The driver gives only an exact, active-thread Takeform MCP approval a positive response. The metadata must name `mcp_tool_call`, identify the next operation in the one-snapshot-then-one-proposal sequence, contain an explicit exact `tool_params` object, and use the supported empty-object form schema. Null, pre-turn, wrong-thread, repeated, reordered, or content-requiring requests are declined. The current [Codex app-server source](https://github.com/openai/codex/blob/main/codex-rs/core/src/session/mcp.rs) uses the same approval metadata and accepts approved MCP calls with empty content.
