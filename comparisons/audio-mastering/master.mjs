...
function timing(audio, video) {
  const numberOrZero = value => finite(value) ? Number(value) : 0;
  return { channels: audio.channels, sampleRate: Number(audio.sample_rate), durationSeconds: numberOrZero(audio.duration), audioStartSeconds: numberOrZero(audio.start_time), videoStartSeconds: numberOrZero(video.start_time), avStartOffsetSeconds: numberOrZero(audio.start_time) - numberOrZero(video.start_time) };
}

export async function masterArtifact({ rawPath, outputPath, attemptRoot, binaries, policy, videoSampleFrames, signal } = {}) {
  ...
  assert(policy && finite(policy.targetIntegratedLufs) && finite(policy.maxTruePeakDbtp) && finite(policy.loudnessRangeTarget) && finite(policy.measurementResolution) && finite(policy.maxPrimingOffsetSeconds), 'declared mastering policy is incomplete', 'invalid-policy');
  ...
  try {
    ...
    receipt.raw.videoIdentity = mediaIdentity(rawProbe.video); receipt.raw.timing = timing(rawProbe.audio, rawProbe.video);
    ...
    const argv = ['-hide_banner', '-y', '-i', rawReal, '-map', '0:v:0', '-map', '0:a:0', '-c:v', 'copy', '-c:a', policy.audioCodec, '-b:a', policy.audioBitrate, '-ac', String(rawProbe.audio.channels), '-ar', String(rawProbe.audio.sample_rate), '-af', filter, '-movflags', '+faststart', tempOutput];
    ...
    const masteredProbe = await probe(binaries.ffprobePath, tempOutput); receipt.commands.push(masteredProbe.command); receipt.mastered = { streams: masteredProbe.data.streams, videoIdentity: mediaIdentity(masteredProbe.video), timing: timing(masteredProbe.audio, masteredProbe.video) };
    ...
    receipt.priming = { durationDifferenceSeconds: masteredTiming.durationSeconds - rawTiming.durationSeconds, avOffsetDifferenceSeconds: masteredTiming.avStartOffsetSeconds - rawTiming.avStartOffsetSeconds, boundSeconds: policy.maxPrimingOffsetSeconds };
    assert(Math.abs(receipt.priming.durationDifferenceSeconds) <= policy.maxPrimingOffsetSeconds && Math.abs(receipt.priming.avOffsetDifferenceSeconds) <= policy.maxPrimingOffsetSeconds, 'encoder priming/edit-list difference exceeds declared bound', 'timing-out-of-bounds', receipt);
    await rename(tempOutput, outputPath); receipt.mastered.sha256 = await sha256(outputPath); receipt.status = 'succeeded'; receipt.elapsedMs = Date.now() - started;
    return receipt;
  } catch (error) {
