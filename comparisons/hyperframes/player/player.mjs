...
function mLayers(snapshot) {
  ...
  for (let frame = 0; frame < frameCount; frame += 1) {
    ...
    verificationFrames.push({frame, media, pictures: state.pictureLayers.map(layer => ({occurrenceId: layer.occurrenceId, geometry: layer.geometry, opacity: layer.opacity}))});
  }
...
function compositionHtml(model, assets) {
  ...
  const elements = model.layers.map(({occurrence}) => {
    ...
    const attributes = `id="layer-${cssId(occurrence.id)}" class="layer" src="${asset.path}" data-start="${outputStart}" data-duration="${outputDuration}" data-media-start="${sourceStart}" data-playback-rate="${playbackRate}" data-takeform-occurrence-id="${occurrence.id}"${media}`;
    return tag === 'video' ? `<video ${attributes}></video>` : `<img ${attributes}>`;
  }).join('\n');
...
function indexHtml(snapshotId, model) {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Takeform HyperFrames M Player</title><style>html,body,#stage{margin:0;width:100%;height:100%;overflow:hidden;background:#101114}hyperframes-player{display:block;width:100%;height:100%}</style><script src="runtime/hyperframes-player.global.js"></script></head><body><main id="stage"><hyperframes-player id="player" src="composition.html" width="1920" height="1080" muted></hyperframes-player></main><script type="module">const snapshotId=${JSON.stringify(snapshotId)};const state=await fetch('verification.json',{cache:'no-store'}).then(response=>response.json());const player=document.querySelector('#player');let displayedFrame=0;const iframe=()=>player.iframeElement;const paint=()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));const close=(actual,expected)=>Math.abs(actual-expected)<.75;function layer(documentInPlayer,id){return documentInPlayer.querySelector('[data-takeform-occurrence-id="'+CSS.escape(id)+'"]');}function activeLayerMatches(element,picture){if(!element)return false;const style=getComputedStyle(element);if(style.display==='none'||style.visibility!=='visible'||!close(Number(style.opacity),picture.opacity))return false;const geometry=picture.geometry;return close(parseFloat(style.left),geometry.x*1920)&&close(parseFloat(style.top),geometry.y*1080)&&close(parseFloat(style.width),geometry.width*1920)&&close(parseFloat(style.height),geometry.height*1080);}function inactiveLayerHidden(element){if(!element)return false;const style=getComputedStyle(element);return style.display==='none'||style.visibility==='hidden'||Number(style.opacity)===0;}async function waitForDisplayedFrame(frame){const target=frame*state.rate.den/state.rate.num;player.seek(target);const deadline=performance.now()+2000;for(;;){await paint();const documentInPlayer=iframe()?.contentDocument;if(!documentInPlayer)throw new Error('composition iframe is unavailable');const expected=state.frames[frame].pictures;const active=new Set(expected.map(picture=>picture.occurrenceId));const layersReady=expected.every(picture=>activeLayerMatches(layer(documentInPlayer,picture.occurrenceId),picture))&&state.occurrenceIds.filter(id=>!active.has(id)).every(id=>inactiveLayerHidden(layer(documentInPlayer,id)));let mediaReady=layersReady;for(const media of state.frames[frame].media){const element=layer(documentInPlayer,media.occurrenceId);if(!element||element.tagName!=='VIDEO'||element.readyState<HTMLMediaElement.HAVE_CURRENT_DATA||Math.abs(element.currentTime-media.sourceTimeSeconds)>.05){mediaReady=false;break;}}if(mediaReady){displayedFrame=frame;return;}if(performance.now()>=deadline)throw new Error('requested frame was not painted');}}function respond(command,status){const response={requestID:command.requestID,sessionID:command.sessionID,snapshotID:command.snapshotID,displayedFrame,playback:player.paused?'paused':'playing',status};window.webkit?.messageHandlers?.takeformPreview?.postMessage(response);return response;}let commandTail=Promise.resolve();function serialize(command){const run=commandTail.then(command,command);commandTail=run.catch(()=>{});return run;}window.takeformPreviewCommand=command=>serialize(async()=>{if(!command||command.snapshotID!==snapshotId)throw new Error('snapshot identity mismatch');if(command.type==='load'||command.type==='seek'){if(!Number.isInteger(command.frame)||command.frame<0||command.frame>=state.frameCount)throw new Error('frame outside snapshot');await waitForDisplayedFrame(command.frame);return respond(command,'painted');}if(command.type==='play'){player.play();await paint();return respond(command,'painted');}if(command.type==='pause'){player.pause();await paint();return respond(command,'painted');}throw new Error('unsupported preview command');});if(!player.ready)await new Promise(resolve=>player.addEventListener('ready',resolve,{once:true}));await waitForDisplayedFrame(0);</script></body></html>`;
}
...
export async function buildMPlayerBundle({bundleRoot, derivativeRoot, fixtureRoot, mediaPrepManifestPath, mediaPrepModulePath, runtime}) {
  ...
  await writeFile(join(root, 'verification.json'), json({frameCount: model.frameCount, frames: model.verificationFrames, occurrenceIds: model.layers.map(layer => layer.occurrence.id), rate: model.rate, snapshotId: snapshot.snapshotId}));
