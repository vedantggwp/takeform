...
The fixture defines `first-last-frame-hash-stable` as identical first and last decoded-frame hashes across two renders on one machine. The first decoded RGBA hash was the same in all three runs, but final hashes differed (`0ae049…`, `c36bd8…`, `f9be9b…`). The predicate therefore **fails**. Independent inspection saw no visible sampled content, timing or layout discrepancy, which does not establish an encoding explanation. The authorized lossless still diagnostic failed during public browser discovery before its first capture; it produced no PNG, bundle fingerprint, or decoded hash. The three outputs remain retained.
