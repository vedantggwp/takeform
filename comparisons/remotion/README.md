...
`render-attempt.mjs` will use the reviewed runtime's public `bundle()` and `renderMedia()` APIs. It converts rational FPS to a number only at Remotion's API boundary. It requires explicit streaming reserve inputs and uses one render worker with the accepted browser wrapper in `chrome-for-testing` mode.
