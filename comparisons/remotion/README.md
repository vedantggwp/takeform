...
`render-attempt.mjs` uses the reviewed runtime's public `bundle()` and `renderMedia()` APIs. It converts rational FPS to a number only at Remotion's API boundary. It requires explicit streaming reserve inputs and uses one render worker with the accepted browser wrapper in `chrome-for-testing` mode. A caller must launch every render candidate in an attempt-owned Node child with `TMPDIR` set to that attempt's `tmp` directory before this module imports either SDK. The adapter rejects any other `TMPDIR`. Pinned Remotion creates its internal browser profile under `os.tmpdir()` and deletes it on browser close or process exit.
