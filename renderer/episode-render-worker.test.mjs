...
import {compositionModule, runAttempt, validateRequest, validateRuntime, writeProject} from './episode-render-worker.mjs';
...
test('runtime rejects a forged pinned package identity before a renderer import', async () => {
  ...
  try {
    ...
    await Promise.all(packages.map(async ([path, name, version]) => {
    ...
    const runtime = {runtimeRoot: root, nodeVersion: process.version, browserExecutable: browser, ffmpegExecutable: ffmpeg, ffprobeExecutable: ffprobe};
...
test('runtime preflight failure writes a failed receipt before project staging or SDK import', async () => {
  ...
  try {
    ...
    value.runtime = {runtimeRoot: join(root, 'missing'), nodeVersion: process.version, browserExecutable: join(root, 'browser'), ffmpegExecutable: join(root, 'ffmpeg'), ffprobeExecutable: join(root, 'ffprobe')};
